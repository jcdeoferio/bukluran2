		...
		foreach($result as $row){
			...
			switch($row['emailtypeid']){
				case MEMBER_CONFIRMATION_EMAIL:		
					...
					$data['organization'] = $this->organization_model->get_organization_profile($row['organizationid'],$this->variable->current_application_aysem());
				...
				case FACULTY_CONFIRMATION_EMAIL:
					$data['faculty'] = $this->faculty_model->get_profile($row['facultyid']);
					$data['organization'] = $this->organization_model->get_organization_profile($row['organizationid'],$this->variable->current_application_aysem());
